...
@pytest.mark.asyncio
async def test_core_fetch_uses_normalized_path_and_auth_headers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("ai_home_copilot.entity.async_get_clientsession", lambda _hass: _Session())
    ...
    data = await entity._fetch("api/v1/hub/light")

    assert data == {"ok": True, "value": 1}
    assert captured["url"] == "http://supervisor:8909/api/v1/hub/light"
...
@pytest.mark.asyncio
async def test_core_fetch_returns_none_on_non_200(monkeypatch: pytest.MonkeyPatch) -> None:
    class _Resp:
        status = 503
    ...
    assert await entity._fetch("/api/v1/hub/light") is None
