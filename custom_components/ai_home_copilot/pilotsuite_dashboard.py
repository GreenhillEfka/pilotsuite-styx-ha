...
async def async_generate_pilotsuite_dashboard(hass: HomeAssistant, entry: ConfigEntry) -> Path:
    ...
    intro = _markdown_card(
        "Welcome",
        """
This dashboard is generated by AI Home CoPilot (governance-first).

Quick links:
- Core health: `http://<HA_IP>:8909/health`
- Core version: `http://<HA_IP>:8909/version`
- Core events: `http://<HA_IP>:8909/api/v1/events`

Tip: Use the buttons below to fetch capabilities/events and to surface HA errors as notifications.
""",
    )
    ...
    grid = _grid_card(
        [
            ...
            _entities_card("Ops (safe)", operations_entities),
            _entities_card("Dashboards", dashboards_entities),
    ...
    views.append(
        _view(
            ...
            "\n\n".join(
                [
                    _entities_card("Copilot-Core tools", core_entities),
                    _markdown_card(
                        "Notes",
                        """
If `/api/v1/events` stays empty:
- check `forwarder status`
- toggle a light that is inside a Habitus zone
- use `Fetch HA errors` for thread-safety warnings or POST errors.
""",
                    ),
                ]
            ),
