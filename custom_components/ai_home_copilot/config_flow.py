        ...
        schema = vol.Schema(
            {
                ...
                # Note: Token is intentionally plain string, not password.
                # Home Assistant's options flow doesn't support password masking well.
                # For security: the token is only transmitted to Core Add-on, never logged.
                vol.Optional("_clear_token"): bool,
                # Checkbox to clear existing token. Only shows if token is currently set.
