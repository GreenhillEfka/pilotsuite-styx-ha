...
STORAGE_VERSION = 1
STORAGE_KEY = f"{DOMAIN}.systemhealth"
...
def _store(hass: HomeAssistant) -> Store:
    ...
    st = global_data.get("systemhealth_store")
    if st is None:
        st = Store(hass, STORAGE_VERSION, STORAGE_KEY)
        global_data["systemhealth_store"] = st
    return st


async def async_get_state(hass: HomeAssistant) -> SystemHealthState:
